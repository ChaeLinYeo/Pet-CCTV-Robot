...
def mouse_callback(event, x, y, flags, param):
    global danger_done, set_danger_box
    if event == 1 : 
        print( '{0} ,{1}'.format(x,y))
        danger_done +=1
        set_danger_box.append((x,y))
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            if(labels[int(classes[i])] == 'cat' or labels[int(classes[i])] == 'dog'):
                ...
                for i in range(0,len(set_danger_box),2):
                    if((xmin < set_danger_box[i][0] < xmax and ymin < set_danger_box[i][1] < ymax) or ((xmin < set_danger_box[i+1][0] < xmax and ymin < set_danger_box[i+1][1] < ymax))):
